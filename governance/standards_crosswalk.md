...
| Structured governance explanations | `explainer.py`, `schemas.py`, `tests/test_explainer.py` | Measure, Govern | Explainability and traceability | Processing integrity | Auditability, minimum necessary | Overreliance, sensitive disclosure | Reviewer rationale without Chain-of-Thought |
| Prior-auth evidence coverage report | `evidence_coverage.py`, `tests/test_evidence_coverage.py`, `examples/prior_authorization/evidence_coverage_sample.json` | Measure, Manage | Explainability and lifecycle controls | Processing integrity | Auditability, minimum necessary | Overreliance, insecure output handling | Documentation coverage support without approval/denial |
| HITL escalation and closure | `escalation.py`, `review.py`, `scripts/record_human_review.py`, `tests/test_human_review.py` | Govern, Manage | Human oversight | Control activities | Workforce/process safeguards | Overreliance | Prior-auth decision-support boundary |
| PHI-safe audit trace and hash-chain verification | `audit.py`, `audit_store.py`, `audit_hashing.py`, `scripts/verify_audit_chain.py` | Govern, Measure | Monitoring and evidence | Logging/monitoring, processing integrity | Audit controls | Logging and monitoring | Traceability to source documents |
...
| Optional governance observability | `telemetry.py`, `docs/observability.md`, `tests/test_telemetry.py` | Measure, Manage | Monitoring | Monitoring, availability | Audit-adjacent monitoring | Logging and monitoring | Operational traceability without PHI attributes |
